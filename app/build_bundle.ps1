...
# ANY existing value, not just an empty one. The build machine is itself an
# updated install now, so its source arrives already carrying the date of the
# LAST build - and matching only "" made the first build after midnight throw
# "BUILD_DATE was not stamped" while yesterday's date sat right there.
