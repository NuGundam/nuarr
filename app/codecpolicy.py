    ...
    "audio": [
        ...
        {"key": "copy_stereo_if", "label": "Stereo formats to copy",
         ...
         "note": "eac3 is deliberately absent, and stereo is the reason - the "
                 "same codec is ticked for surround one row up. Not because "
                 "anything refuses it: across this server's whole play "
                 "history, stereo E-AC3 has direct-played on every device that "
                 "met it and has never once been transcoded. Two other things "
                 "are wrong with it. FIRST, Sony's Bravia app advertises "
                 "E-AC3 and then the TV stumbles on the stereo variant, so "
                 "Plex falls back to transcoding the audio mid-stream and a "
                 "skip-intro seek restarts that transcode and times out - the "
                 "failure that started this. SECOND, a direct-played E-AC3 "
                 "track is decoded by the CLIENT, and a client's Dolby decoder "
                 "applies its own dialogue-level handling, so the same episode "
                 "plays several dB louder as E-AC3 than as AAC on the same "
                 "laptop. AAC carries no such field and is simply played as "
                 "written. The conversion itself costs nothing, measured: a "
                 "Crunchyroll DDP 2.0 episode and nuarr's AAC of it are both "
                 "-22.3 LUFS integrated, the same to a tenth of a decibel. And "
                 "AAC stereo is the best-evidenced format on this server - 630 "
                 "plays across 40 devices against 7 transcodes - where E-AC3 "
                 "5.1, by comparison, has been transcoded by 19 of them. Tick "
                 "eac3 here and the panel below will immediately tell you what "
                 "every device does with stereo E-AC3, because the question it "
                 "answers is what these settings actually produce."},
